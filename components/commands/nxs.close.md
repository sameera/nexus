...
the hub names **every distinct repo** the migrated entry's `range:` list names (epic #214, story
#508 — never only the first entry's repo), aged from the migration commit. In single-repo and
hub mode no migration happens and #172's tmp-only behavior applies unchanged.
...
(`nexus config backlog-query`), so a repository that
