...
publishes a local file into it, and what the pinned reference it gets back looks like — including the
renderer an HTML mockup is referenced through (epic #613). Load it from
...
the body with a reference pinned to the commit the publish created. An image shows inline; an HTML
page is referenced through the team's configured renderer, so a reviewer opens the mockup rather
than its markup; every other file is a link at that commit. A later upload to the same path makes a
new commit and never changes what an earlier issue shows, so file names need no coordination across
epics and the store is never deleted from.

## Configuration

All three keys live in the `github:` block of `.nexus/config/settings.yml` — the same block as
`issues-repo` — so they ride the same precedence chain and the same hub layer. A hub may declare any
of them in its `workspace.yml` `github:` defaults; a member that declares none inherits them.
...
github:
    ...
    asset-renderer: "https://preview.example.com/?url={url}"   # HTML mockups; no default
```
...
- `asset-renderer` — the address template an **HTML** asset's reference is built from. **No default.**
  It must be an absolute `http`/`https` address carrying exactly one `{url}` slot; the asset's
  commit-pinned address is substituted into that slot **verbatim**, neither escaped nor
  percent-encoded, so a template a lead verified in a browser is the value they paste here. A
  declared value that is not an absolute web address, or that carries no slot, **stops the run at
  intake** (`assets malformed-renderer:`), before any draft exists. A repository that declares none
  files the plain link it files today. The choice between the two forms turns on this key alone and
  never on the store's visibility — a renderer must be able to read the store, so a private store
  needs the reader epic #614 builds.
...
# https://preview.example.com/?url=https://github.com/acme/assets/blob/<commit>/features/<slug>/mock.html
```
...
- The reference names the **commit** the publish created, never a branch — inside the renderer's
