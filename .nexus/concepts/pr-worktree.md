...
The pull-request post-merge flow runs its stages in an isolated worktree, not the lead's own checkout. Where worktrees are created is declared configuration through a publishing key. This key names a base directory, resolved through the same precedence chain every other publishing key uses. When nothing is declared, the base defaults to a system-temp location. An unusable base is refused with a named diagnostic before anything is created.

## How It Works

Conformance opens a detached worktree at the pull-request head. Closure opens one on a fresh distillation branch cut from the trunk, and distillation continues in it. Every opening path runs through one private step that resolves the base, runs the gate, and creates the directory. Both stages resolve identically within a run, so the gate cannot be bypassed.

The base comes from the shared publishing resolver, asked for that one key and anchored on the checkout's root. The checkout root is an operator-supplied directory, defaulting to the invoking one and normalized through the version-control system's own top-level answer before anything is selected. An empty answer means undeclared and takes the built-in temp-derived base. If the resolver fails, the run stops instead of silently writing a commit-bearing checkout into the very directory the operator had configured away from. A declared value is trimmed, unquoted, home-expanded, and made absolute against the repo root.

A per-checkout segment is appended underneath. Reuse is path-based. Removal is force-and-prune, run from the checkout the operator pointed to.
...
3. The base is resolved from the operator-pointed checkout's root, never from pull-request content. A fork head must not choose where a checkout is written. The root is itself normalized through git's own top-level answer before it is trusted.
