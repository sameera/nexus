...
A named form is resolved against the toolkit's own declared surface, obtained from that surface and never from a copy; a surface that cannot be obtained fails the gate rather than falling back to an assumed list. Both surfaces are now the same language, so the gate reads each as a value and executes nothing.
...
4. The gate executes nothing — it reads both declared surfaces as values — and needs no network access, no credentials, and never a string read from a body.
