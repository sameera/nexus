...
- **Git-tracked, not derived.** These pages hold distilled *judgment* (the "why") that cannot
  be regenerated from code — so they are versioned and reviewed like source. The
  `.nexus/anchors/` sidecars are the derived half: SHA-stamped file-path maps, regenerated by
  the distiller on every touching drain, never hand-edited.
