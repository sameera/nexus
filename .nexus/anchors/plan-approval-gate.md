...
- `libs/portable-tools/src/plan-approval.ts` — the coverage refusal that recomputes rather than trusts, the gate digest printed line by line, and the rebuild after a mark change (#592)
- `libs/portable-tools/src/plan-approval.spec.ts` — pins the three refused drafts, every element of the digest, and a mark changed and changed back giving the same draft (#592)
