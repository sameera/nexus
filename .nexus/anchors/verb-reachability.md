...
- `libs/portable-tools/src/build-bundles.ts` — the entry-point map, one runnable artifact per declared toolkit name; the launcher registrations stay gone.
...
- `libs/delivery-config/src/registry.ts` — the second toolkit's registry now: one row per capability, the composed usage text, and the sorted machine listing, with two rows whose runnables delegate across a process boundary.
...
- `libs/delivery-config/src/delegate.ts` — what such a row's runnable does: runs a retained entry point and forwards its exit code, leaving what the registry declares unchanged.
