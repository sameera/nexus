...
## 2026-09-12 — A rerun normalizes the draft back to one slice per story, rather than teaching the passes about parts
- **Choice:** `rewritePlan` merges a story's parts back into the slice they were split from and drops the scaffolds before any pass runs, so every pass keeps reading the one-slice-per-story shape epic #456 writes.
- **Why:** The rewrite is run again over its own output — the recorded declaration mapping exists so a later run needs no `--declare` — and a part carrying its siblings' concepts in `assumes` is indistinguishable from another story, which made `decideScaffolds` scaffold every concept an earlier part taught and the greedy pass order part 2 first. Normalizing is one place; making ordering, scaffolding and coverage each part-aware is four.
- **Refuted alternative:** Keep the parts and teach the passes about them — key `placed` on story-plus-part, let `decideScaffolds` accept a sibling part as an introducer, and constrain the greedy to keep parts consecutive. That preserves the already-split shape but spreads the split contract across three passes and reintroduces the ordering feedback record #562 removed.
