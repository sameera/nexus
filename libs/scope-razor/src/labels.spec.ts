...
import { stripLabels, survivingLabels, type Finding } from "./labels.js";
...
describe("asserting a derived body carries no drafting-time token", () => {
    it("reports nothing for a body with no label", () => {
        expect(survivingLabels("- The user can log out\n- Sessions expire\n")).toEqual([]);
    });

    it("names the line and the token of a surviving asked label", () => {
        const found: Finding[] = survivingLabels("# Epic\n\n- One `[asked: \"one thing\"]`\n");
    ...
    it("names every surviving label, not just the first", () => {
        expect(survivingLabels("- One `[inferred]`\n- Two `[inferred]`\n").map((f: Finding) => f.line)).toEqual([1, 2]);
    });

    it("does not report ordinary bracketed prose as a label", () => {
        expect(survivingLabels("A sentence with [brackets] and [NEEDS CLARIFICATION: x] in it.")).toEqual([]);
    });
});
