...
### Story #378: Arguments, target root and prerequisites settle before anything is written

**As a** delivery lead filing an epic, **I want** every bad invocation refused before the first remote call, **so that** a mistyped path or a wrong working directory costs me an error message rather than an issue filed into the wrong repository.
