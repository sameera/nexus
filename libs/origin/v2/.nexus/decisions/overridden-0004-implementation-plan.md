# 0004 — Refactor implementation plan (OVERRIDDEN — historical)

> **This is the pre-supersession original, retained for history.** Its storage and
> handoff model (the `docs/`-resident close record, the `.temp`+`staged` surfaces, the
> close-emission BLOCKER, the `ConceptDelta`-block-in-close-record) was overtaken by
> [`0005`](./0005-transient-artifact-storage.md), [`0006`](./0006-queue-distillation-handoff.md),
> and [`0007`](./0007-delta-in-pr-merge-apply.md). **The current plan, with those folded
> in, is [`0004-implementation-plan.md`](./0004-implementation-plan.md).** Read that;
> this file only preserves the superseded reasoning.

**Superseded in part by:** [`0005`](./0005-transient-artifact-storage.md),
[`0006`](./0006-queue-distillation-handoff.md) — storage/emission model changed (a committed
`.nexus/queue/` replaces the `docs/`-resident close record and the `.temp`+`staged` surfaces);
the **close-emission BLOCKER is dissolved** (durability now structural); A0's
`close-record-template` loses its machine-handoff half.
...
**or** machine-consumed per the 0003 contract (→ `.nexus/concepts/`). No artifact serves both
stores. Two pre-emptive razor checks, because both look like violations and aren't:

- **The close record lives in `docs/` and carries a `ConceptDelta` block.** Not a violation:
  the deltas are human-gated _at the close review_ (the forcing function), and the distiller
  _reads_ the gated record — the same way `/nxs.hld` reads concept pages. A machine block a
  human approves before it crosses the wall is a handoff, not a shared artifact. The
  violation the wall prevents is _ungated_ machine blobs in `docs/` or human prose in the
  concept store (open-notebook note §4.2, `save_as_note()` — never build the laundering path).
- **`concepts:` reading-list frontmatter on System-A artifacts** points _into_ the concept store
  but stores nothing from it — a read pointer, not relocated content. Allowed.

If any work item below would emit a human artifact into the concept store or an ungated machine
...
- `claude/.claude/nexus/templates/decision-record-template.md` — replaces the 16-section HLD.
  ...
  Constraints/Invariants incl. security (4+9
  slimmed) · Blocker/Address risks (13 slimmed) · Open clarifications. Each section
  annotated with which `ConceptDelta` field it feeds at close (invariants→`invariants_added`,
...
- `claude/.claude/nexus/templates/close-record-template.md` — replaces `PIR.md`. Human-facing
  half: key decisions, deferred-scope pointer. Machine handoff half: the per-concept
  `ConceptDelta` block (0003 §8.2 shape), one entry per affected concept, each with its
...
| C3 — G1          | Home for process/delivery lessons (estimate-vs-actual, decomposition lessons)   | **Decided (0002 §b G1, 2026-06-19 — overrides the earlier "Gotchas in command docs" default):** out of concept-store scope; a `docs/system/delivery/lessons/` folder, **one file per lesson** (`<date>-<slug>.md`, source-epic in frontmatter), written by `/nxs.close`. Folder-of-files not a single ledger — adds-a-file is merge-conflict-free across concurrent epics, mirroring the concept store's one-concept-per-file stance (0003 §6/§7). Permanent human `docs/` artifact (consumed by PM estimation). PAI §3's Gotchas pattern survives only as the per-entry *shape*, not the home. | `docs/system/delivery/lessons/` (new folder) + `/nxs.close` write |
...
to Phase C per C9); C1–C7 resolved and reflected in them; each template
section that survives into a `ConceptDelta` is annotated with its receiving field (proves the
...
| `nxs.hld.md`             | **Rewrite** — emits the focused **decision record** (A0 template), not the 16-section HLD. Consumes concept pages for System Context instead of regenerating them (in B3 this read goes live; until then it is a manual/README-driven read).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | §4       |
...
| `nxs.close.md`           | **Rewrite** — emits the **close record** into the committed queue (per 0006: human prose only — key decisions + deferred-scope pointer + deviation rationale; **no `ConceptDelta` block**, BLOCKER dissolved). Also writes the G1 process lesson as a new file in `docs/system/delivery/lessons/` (C3) and appends the G3 deferred scope to `docs/features/<feature>/backlog.md` (C2), then points the close record at that backlog. Drop prose PIR. Keep GH comment + `gh issue close`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | §9       |
...
- `CLAUDE.md` (repo root) — **Rewrite** the command-workflow section: remove dev/qa stages,
  ...
  the two-store split. This file describes the _pre-refactor_ pipeline (task warning) — it is
...
**BLOCKER — close emission must land somewhere durable.** `/nxs.close` emits `ConceptDelta`s
but B's queue/apply doesn't exist until B1. Close must write the delta block to a durable
staged form now — the `ConceptDelta` block inside the close record itself, plus (preferred)
...
describes the lean set; update script no longer leaves orphans. Concept store still empty — no A1
work writes to `.nexus/concepts/`.
...
slim GH issues, an inline analyze gate, and a close record whose `ConceptDelta` block
...
amendment surface) — _not_ the page schema.
...
- _Candidates never write the concept store directly_ (PAI §2.1): close emits to a staged queue;
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C11 — `touches:` reciprocity | 0003 never says whether a touched page gets a reciprocal `touches_*` edit (PAI §2.5). Without a rule, blast-radius grep under-reports. | **Adopt symmetry:** a real interaction is bidirectional — one delta's `touches_added: [X]` fans out to a reciprocal edit on page X in the _same_ emission. Make it a deterministic distiller step, not prompt judgment. |
| C12 — Queue expiry           | Staged deltas nobody promotes (PAI §2.6).                                                                                              | **Adopt expiry:** unreviewed staged deltas expire (PAI uses 90 days). A queue that only grows is [speculative over-generation](../concepts/speculative-over-generation.md) relocated.                                                                                         |
...
- Staged candidate queue — `ConceptDelta`s land here at close, applied only after curation;
  expiry per C12.
...
- close-record `ConceptDelta` block → queue → curated apply, with C11 reciprocity fan-out
...
decision-log-entry, over-cap body) is _rejected by code_, not by prompt. Reciprocal
...
- **BLOCKER — close emission durability (A1).** If `/nxs.close` discards `ConceptDelta`s
  before B1 exists, A2 yields no real B fixture and B1 starts blind. Mitigation: A1 writes
  the delta block to the close record _and_ a sidecar staging file in 0003 §8.2 shape.
- **BLOCKER — two-store leak via the close record.** The close record (in `docs/`) carrying
  a machine block is safe _only_ while the deltas are gated at the close review. If close
  ever auto-applies to the concept store without that human gate, the wall is breached (open-
...
- **ADDRESS — distiller over-generation (the disease, recursed).** Thousands of concept store
  ...
  Phase C razor audit must cover the distiller's _output_, not just the command pipeline.
