...
PAI now contributes only the compatible _format/shape_, not the home or the cap stance.
...
- **The queue holds committed human planning artifacts that the distiller consumes.** Not a
  ...
  _reads_ the queue and _writes_ concepts; it never writes back into the queue. Because A
...
- `common/templates/decision-record-template.md` (tool-agnostic master; seeded to `.nexus/config/templates/`) — replaces the 16-section HLD.
  ...
  (the _why_) — it is committed in the queue and read by B post-merge. Each section is
...
- ~~`common/templates/task-index-template.md`~~ — **CUT by [0009](./0009-story-as-implementation-unit.md).**
  The task layer is vestigial after the dev-stage cut (the story is the implementation unit; the
  distiller never read the task index). No task index, no per-task LLD, no `story_ref` traceability.
  `/nxs.tasks` instead sequences the epic's stories inline and creates one issue per story.
- `common/templates/close-record-template.md` — replaces `PIR.md`.
  ...
  **No `ConceptDelta` block** — the machine-handoff half is removed; the distiller mines this
  prose for the _why_ it cannot derive from the diff.

**Deliverables — modify:**

- ~~`task-template.md`~~ — **CUT by [0009](./0009-story-as-implementation-unit.md)** along with the
  task layer. The GH issue body is the story itself (As-a / AC / Notes), assembled by `/nxs.tasks`
  from the epic; there is no per-task file template.
...
| #                     | Question                                                                        | Decision (PAI/0002)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Lands in                                                                                                                                                       |
| --------------------- | ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C1 — G2               | Format of `decision_log_entry.body` "why"                                       | **Decided (0002 §b G2, 2026-06-19 — overrides the earlier "format not cap relaxation" default):** relax the 1–3-sentence cap to admit the refuted alternative + why it lost. **Viability guardrail:** record an alternative only if genuinely viable (a competent engineer might have chosen it; rejected on a real trade-off), never a strawman — the guardrail is what keeps the relaxation from reopening over-generation. PAI §3's `conjectured/refuted-by/learned/criterion` framing is the compatible disciplined _format_ for it.                                                                                                                                                                                                                                                                                                                                                 | decision-record template (the human captures the _why_ here) + B1 distiller `decision_log_entry` recipe (per 0006: **B** constructs the delta from this prose) |
| C2 — G3               | Durable home for deferred scope (prose PIR "Future Considerations" is cut)      | **Decided (0002 §b G3, 2026-06-19 — pins the earlier "stub" default to a path):** append-only `docs/features/<feature>/backlog.md` — the feature's re-triage queue and the input the next `/nxs.epic` reads. **Two writers ([0008](./0008-epic-direct-intent-and-stub-decomposition.md)):** `/nxs.close` (close-time deferred scope) **and** `/nxs.epic` (epic-time decomposition stubs), sharing one **common entry shape** (slug + one-line goal + complexity S/M + status). The close record (in queue) carries only a pointer. Stays on the human surface, never concept store (§8.3 speculative).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | `docs/features/<feature>/backlog.md` (new) + close-record pointer                                                                                              |
| C3 — G1               | Home for process/delivery lessons (estimate-vs-actual, decomposition lessons)   | **Decided (0002 §b G1, 2026-06-19; home moved out of `system/` 2026-06-22 — overrides the earlier "Gotchas in command docs" default):** out of concept-store scope; a `docs/delivery/lessons/` folder, **one file per lesson** (`<date>-<slug>.md`, source-epic in frontmatter), written by `/nxs.close`. Top-level `docs/delivery/`, **peer of `system/` not child** — `system/` is system-concept knowledge (what the product is); these lessons fail that test (the §9.1 failure that evicted them from the concept store), so they don't sit under `system/`. Folder-of-files not a single ledger — adds-a-file is merge-conflict-free across concurrent epics, mirroring the concept store's one-concept-per-file stance (0003 §6/§7). Permanent human `docs/` artifact (consumed by PM estimation). PAI §3's Gotchas pattern survives only as the per-entry _shape_, not the home. | `docs/delivery/lessons/` (new folder) + `/nxs.close` write                                                                                                     |
| C4 — G4 — **DECIDED** | Home for cross-cutting NFR budgets (e.g. global "page load < 2s")               | **Decided:** route to `docs/system/standards/`; do **not** invent a synthetic `performance` concept page. Cross-cutting concept pages get blessed only if a real shared concept later emerges.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | standards (decision-record references it)                                                                                                                      |
| C5 — **DECIDED**      | Tier-scale decision-record sections by epic complexity rating                   | **Decided:** tier via an explicit whitelist (_not_ a heuristic — PAI §4.1, heuristic bypass becomes doctrine-evasion): S/M require Decisions + Invariants only; L/XL require all sections. Encoded as an explicit required-section rule in the template.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | decision-record template                                                                                                                                       |
...
| `nxs.tasks.md`           | **Rewrite ([0009](./0009-story-as-implementation-unit.md)).** No HLD-into-tasks decomposition. **Sequences the epic's stories** (dependency order), runs the inline analyze gate, then after a review checkpoint creates **one GH issue per story** (child of the epic). Adds a thin `## Implementation Sequence` to the epic in the queue — no separate task index, no per-task LLD. Keep the interactive Epic Scope Validation gate. Drops `nxs-decomposer` / `nxs-architect`-LLD / `nxs-generate-tasks`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | §5       |
| `nxs.analyze.md`         | **Rewrite ([0009](./0009-story-as-implementation-unit.md)).** An **inline checkpoint**, no persisted `task-review.md`. Keep the severity gate (blocks issue creation on drift) and terminology normalization. **Story↔task traceability and the barrel-merge auto-remediation are dropped** (no tasks to trace or merge). **Story-level checks:** (1) every story has design coverage in the decision record — an uncovered story is a gap; (2) for `story_type: user`, at least one of the story's own ACs is an observable behavioral outcome; (3) for `story_type: system`, at least one AC states a measurable criterion (metric, threshold, or pass/fail) — prose-only ACs ("implement caching") are a severity-gate failure.                                                                                                                                                                                                                                                                                | §8       |
| `nxs.close.md`           | **Rewrite** — emits the **close record into the committed queue as pure human prose**: key decisions + deferred-scope pointer + deviation rationale. **No `ConceptDelta` block** — durability is structural (the queue entry is committed). The close record's in-flight **key decisions** are mined from the **epic + story issue comments + the close review** (C6), _not_ from task files (task layer cut, [0009](./0009-story-as-implementation-unit.md)). Implements the **close-from-diff forcing function**: diff the branch **against the decision record**, auto-derive the _what_, **surface detected deviations**, and force the human to supply rationale **only on those deviations** (targeted, not a blank "write a summary"); that rationale lands in the close record. Also writes the G1 process lesson as a new file in `docs/delivery/lessons/` (C3) and appends the G3 deferred scope to `docs/features/<feature>/backlog.md` (C2) — using the **common append-only entry shape shared with `/nxs.epic`'s decomposition stubs** (slug + one-line goal + complexity S/M + status; `backlog.md` is a two-writer surface, [0008](./0008-epic-direct-intent-and-stub-decomposition.md)) — then points the close record at that backlog. The queue entry records its parent feature (one-direction pointer). Drop prose PIR. **GH ops target the epic issue** via `link` (created at `/nxs.epic`, not the deleted `/nxs.tasks`, [0010](./0010-epic-files-stories-at-approval-gate.md)): **precondition — verify every child story issue is closed; if any remain open, block and report them** (do not auto-close, do not proceed), then keep the GH comment + `gh issue close` on the epic. The entry stays committed and travels to main with the PR; the distiller deletes it on consume. | §9       |
...
| `nxs-analyzer.md`          | **Delete** — `/nxs.analyze` (0009) runs the gate inline (`tools:` has no `Task`); no caller remains. Supersedes the earlier "Modify — inline-gate" plan. Prune step (this row's ADDRESS risk) drops it from existing installs. |
...
| Skill                                             | Action                                                                                                                                                                                                                                                                                                                                                                               |
| ------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `nxs-setup`                                       | **Create** — the interactive product-context interview extracted from the old `nxs.product-context` command (≤5 questions one-at-a-time, PM pattern-recognition inference, lean section set → `docs/product/context.md`). Invoked by `/nxs.setup` during bootstrap, and reusable standalone to refresh the product context without re-bootstrapping.                                    |
| `nxs-gh-create-epic`                              | **Keep** — provenance/tracking surface.                                                                                                                                                                                                                                                                                                                                              |
| `nxs-gh-create-task` → `nxs-gh-create-story`      | **Repurpose ([0009](./0009-story-as-implementation-unit.md))** — creates one GH issue per **story** (`STORY-*.md` work-item files emitted to scratch by `/nxs.tasks`), child of the epic. Same frontmatter-driven script; glob + naming changed task→story.                                                                                                                            |
| `nxs-abs-doc-path`                                | **Keep** — utility.                                                                                                                                                                                                                                                                                                                                                                  |
| `nxs-generate-tasks`                              | **Delete ([0009](./0009-story-as-implementation-unit.md))** — task layer cut; nothing emits task files.                                                                                                                                                                                                                                                                               |
...
Run the reshaped pipeline (setup→epic→hld→analyze→close) end-to-end on **one real
...
**Exit criteria:** one epic produces a slim epic, a focused decision record, an epic issue
with child story issues ([0009](./0009-story-as-implementation-unit.md)/[0010](./0010-epic-files-stories-at-approval-gate.md);
no task index), an inline analyze gate, and a **committed `.nexus/queue/<branch>/<local-id>/`
...
- **Input is the committed queue + the diff (0006).** The distiller scans `.nexus/queue/**`
  ...
  the _what_ from the merged diff (SHA range inferable from the merge commit, else recorded
  base/head in the entry; the diff is recomputed, never stored) and the _why_ from the queued
...
| #                                          | Question                                                                                                                               | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| C11 — `touches:` reciprocity — **DECIDED** | 0003 never says whether a touched page gets a reciprocal `touches_*` edit (PAI §2.5). Without a rule, blast-radius grep under-reports. | **Decided: adopt symmetry.** A real interaction is bidirectional — one delta's `touches_added: [X]` fans out to a reciprocal edit on page X in the _same_ distillation-PR. Deterministic distiller step, not prompt judgment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| C12 — Queue expiry — **DECIDED**           | Committed queue entries merged to main but never drained into a distillation-PR (PAI §2.6).                                            | **Decided: no hard expiry — drain-SLO + flag instead** (overrides the PAI 90-day default). Deletion stays bound to **consume** (B1: an entry is deleted only when its distillation-PR merges); undrained entries are **never silently deleted**. An undrained entry past an age threshold is **flagged as a drain-SLO breach** — surfaced via the undrained-queue-age metric attachment point (Phase C). **Why the PAI analogy doesn't transfer:** queue entries are produced **1:1 with merged epics**, each already gated by pipeline review — a growing queue is an unprocessed backlog of real shipped work, _not_ [speculative over-generation](../concepts/speculative-over-generation.md). The thing that risks loss is the entry's unique payload — the decision/close-record **_why_** (refuted alternatives, deviation rationale), the non-regenerable judgment of 0001 D3; the _what_ is never lost (it's the merged code). A slow-growing queue signals a **drain-cadence (System B) failure**, fixed by draining, not by deleting curated input. The distillation-PR is the drain; its cadence is an operational SLO (0007). |
| C13 — Bootstrap trust — **DECIDED**        | How `last_updated_by: "bootstrap"` pages are treated (PAI §1.5).                                                                       | **Decided: adopt low-trust.** Bootstrap pages are re-validated at the first close that touches them; distiller prioritizes that re-validation.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
the optional **structured Decision Log stub** (upgrade A's _why_ capture from free prose to a
stub the human authors directly so B carries it verbatim — compatible with the wall, does not
pull the _what_ pre-merge; pursue or drop in B-phase). See also C6.
...
- Queue reader + delta constructor — scans `.nexus/queue/**` for unconsumed entries, derives
  the _what_ from the recomputed branch diff and the _why_ from the queued decision + close
  records (0006), and constructs `ConceptDelta`s (B's internal representation). **No expiry-delete
  of undrained entries (C12)** — deletion is bound to consume; undrained age is flagged as a
  drain-SLO breach, not deleted.
...
- **Success metrics** — deferred by choice (0001). _Note_ where they could attach (e.g.
  pages-per-close, duplicate-concept rate, distillation-PR drain latency / undrained-queue-age
  (the C12 drain-SLO signal), decision-record section-fill under C5) — **do not invent gates.**
...
- **ADDRESS — distillation-PR backlog.** If distillation-PRs aren't drained promptly a review
  backlog forms (0007). Per C12, undrained queue entries are **flagged as a drain-SLO breach,
  not expired/deleted** (curated _why_ is never discarded); the distillation-PR is the drain, so
  its cadence is an operational SLO.
...
- No technical task layer ([0009](./0009-story-as-implementation-unit.md)) — the **story** is the
  implementation unit and the GH-issue granularity; no task index, no per-task LLD, no
  `story_ref` traceability. The engineer decomposes a story into work with their own tools.
