...
3. **Atlas regeneration.** Rebuild the human orientation page from the store's current
   state:
...
4. **Validator.** Run it over every page the entry changed (staged working-tree state vs the
   last commit):
    ...
    append-only log history, §8.3 rejections, and slug = filename. The second checks the
...
5. **Remove the consumed entry, then commit it together with its pages + anchors** so the deletion
...
- **No machinery**: no recipe/template files, no state file, no retrieval index (0003 §7 —
  ...
  regenerated by `pnpm nexus:generate-atlas`, never a retrieval surface). Idempotency is
  structural: entry presence = unconsumed.
