...
5. **Validator.** Run it over every page the entry changed (staged working-tree state vs the
   ...
   - Hub — each changed page path its own quoted argument, never shell-interpolated:

    ```bash
    node .nexus/tools/validate-concepts.mjs --base HEAD "<changed-page-path>" "<changed-page-path>"
