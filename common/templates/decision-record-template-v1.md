<!--
DECISION RECORD TEMPLATE — OLD FORMAT (v1).

WHEN THIS IS USED
    Only for a --revise of a record that was approved in this format: one whose body has a
    "Constraints & Invariants" section. A revision keeps the format its record was approved in,
    so its frozen lines still match. Every other record is drafted from
    decision-record-template.md, the approval-first format.
