 ...
 *
 * Usage:
 *   npx tsx utils/generate-atlas.ts [--concepts-dir <dir>] [--out <path>] [--check]
 *
...
function main(): void {
    ...
    if (options.check) {
        if (!fs.existsSync(options.out)) {
            console.error(`Atlas missing: ${options.out} (run pnpm nexus:generate-atlas)`);
            process.exit(1);
        }
        const existing: string = fs.readFileSync(options.out, "utf8");
        if (existing !== atlas) {
            console.error(`Atlas out of sync: ${options.out} does not match generated content (run pnpm nexus:generate-atlas)`);
            process.exit(1);
        }
        console.log("OK");
        return;
